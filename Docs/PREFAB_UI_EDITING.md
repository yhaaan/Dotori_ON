...
패널은 `WindowBackground`의 자식이 아니라 형제입니다. 패널이 캔버스 맨 위 150px를 차지하고 `WindowBackground`가 그만큼 아래로 밀리는 구조라, 기존 레이아웃은 서로의 상대 위치를 그대로 유지한 채 통째로 내려갑니다. 패널 높이와 `WindowsOverlayWindow.AvatarPickerPanelHeight`는 같은 값이어야 하며 `PrefabAssetTests`가 둘을 함께 고정합니다.
